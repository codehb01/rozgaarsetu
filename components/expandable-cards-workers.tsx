      ...
      {workers.map((w) => {
        ...
        return (
          <motion.li key={w.id} layout className="">
            <motion.div layoutId={`card-${w.id}-${id}`} ref={ref} className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden">
              <motion.button
                layout
              ...
              <AnimatePresence>
                {isActive && (
                  ...
                  >
                    <div className="text-sm text-gray-700 dark:text-gray-300 mb-3">{w.workerProfile?.bio ?? 'No additional details.'}</div>
