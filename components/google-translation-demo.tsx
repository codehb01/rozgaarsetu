...
import { useGoogleReviewTranslation, useGoogleJobTranslation, useGoogleBioTranslation } from '@/hooks/use-google-translation';
...
function ReviewCard({ review }: { review: typeof mockReviews[0] }) {
  const { translatedText, isLoading, error } = useGoogleReviewTranslation(review.text, review.id);
  ...
  return (
    <Card className="p-4 mb-3">
      ...
      <div className="text-gray-700">
        {isLoading ? (
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-500">Translating with Google...</span>
          </div>
        ) : (
          <>
            <p>{translatedText}</p>
      ...
      <Badge variant="secondary" className="mt-2 text-xs">
        🔸 Google Translate {isLoading ? '(Loading...)' : error ? '(Error - Showing Original)' : '(Cached)'}
      </Badge>
    </Card>
  );
}
...
function JobCard({ job }: { job: typeof mockJob }) {
  const { translatedText: translatedDescription, isLoading: isDescriptionLoading, error: descriptionError } = useGoogleJobTranslation(job.description, job.id);
    ...
    <Card className="p-4 mb-4">
      ...
      <div className="mb-3">
        ...
        {isDescriptionLoading ? (
          <div className="flex items-center space-x-2">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div>
            <span className="text-sm text-gray-500">Translating description with Google...</span>
          </div>
        ) : (
          <>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          ...
          <Badge variant="secondary" className="text-xs">
            🔸 Google: {isDescriptionLoading ? 'Loading...' : descriptionError ? 'Error - Original' : 'Cached'}
          </Badge>
        </div>
      </div>
    </Card>
...
function WorkerCard({ worker }: { worker: typeof mockWorker }) {
  const { translatedText: translatedBio, isLoading: isBioLoading, error: bioError } = useGoogleBioTranslation(worker.bio, worker.id);
  const { t } = useLanguage(); // Static JSON translations for UI elements
    ...
    <Card className="p-4 mb-4">
      ...
      <div className="mb-3">
        ...
        {isBioLoading ? (
          <div className="flex items-center space-x-2">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div>
            <span className="text-sm text-gray-500">Translating bio with Google...</span>
          </div>
        ) : (
          <>
      ...
      <div className="flex justify-between">
        ...
        <Badge variant="secondary" className="text-xs">
          🔸 Google: {isBioLoading ? 'Loading...' : bioError ? 'Error - Original' : 'Cached'}
        </Badge>
      </div>
    </Card>
  );
...
// Main demo component
export default function GoogleTranslationDemo() {
      ...
      <div className="container mx-auto px-6 py-20">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl mx-auto mb-6">
            Experience fast and accurate translations powered by Google Translate with intelligent database caching
          </p>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          ...
          <Card className="p-6">
            ...
            <ul className="text-sm text-gray-600 space-y-2">
              ...
              <li>• High-quality neural translation</li>
              <li>• Real-time with smart caching</li>
              <li>• Used in: Reviews, descriptions, bios</li>
            </ul>
          </Card>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          {/* Reviews Section - Google Translation */}
          <div>
        ...
        <Card className="p-6 mt-6 border-blue-200 bg-blue-50">
          <div className="text-center">
            <h3 className="text-lg font-semibold mb-2 text-blue-800">🔸 Powered by Google Translate</h3>
            <p className="text-sm text-blue-700">
              High-quality neural machine translation with intelligent caching for optimal performance. 
              Translations are cached in the database for faster subsequent loads.
            </p>
          </div>
        </Card>
      </div>
