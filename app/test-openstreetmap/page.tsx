  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Try searching locations and selecting a result.</p>
      </div>
...
{JSON.stringify(selected, null, 2)}
      ...
      <div className="space-y-3">
        ...
        <Button onClick={getCurrentPosition} variant="outline">Use my location</Button>
        <div className="text-sm text-muted-foreground">Status: {status}</div>
        {error && <div className="text-sm text-red-500">{error}</div>}
        {coords && (
          <div className="text-sm">Coords: {coords.lat.toFixed(5)}, {coords.lng.toFixed(5)}</div>
        )}
        {place && (
          <div className="text-sm">Address: {place.displayName}</div>
        )}
      </div>
    </div>
  )
