...
export async function geocodeFreeOSM(query: string, signal?: AbortSignal): Promise<GeocodeResult[]> {
...
export async function reverseGeocodeFreeOSM(lat: number, lon: number, signal?: AbortSignal): Promise<GeocodeResult | null> {
...
function toGeocodeResultFromNominatim(d: any): GeocodeResult {
  const address = d.address ?? {}
  return {
    coords: { lat: parseFloat(d.lat), lng: parseFloat(d.lon) },
    displayName: d.display_name ?? "",
    address: {
      line1: [address.house_number, address.road].filter(Boolean).join(" ") || undefined,
