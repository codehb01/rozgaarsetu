...
export class HybridTranslationService {
  ...
  /**
   * Get translation from cache or translate using Azure and cache the result
   */
  async translateWithCache(
    ...
    try {
      ...
      if (cachedTranslation) {
        ...
        console.log(`✅ 🚀 CACHE HIT! Found cached translation for: "${text.substring(0, 50)}..."`);
      ...
      // Cache miss - translate using Azure
      console.log(`❌ 🔄 CACHE MISS - Translating with Azure: "${text.substring(0, 50)}..."`);
      const azureResult = await this.azureTranslator.translateText(text, targetLanguage, sourceLanguage);
      
      // Extract the translated text from Azure response
      const translatedText = azureResult[0]?.translations?.[0]?.text || text;
...
export const translationService = new HybridTranslationService();
